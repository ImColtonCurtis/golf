...
{
    public static bool levelStarted, levelFailed, levelPassed, fastTap, cheatOn, preGame, reloadLevel, inLoading;
    ...
    bool restartLogic, startLogic, passedLogic, fasterLoad;
    ...
    [SerializeField] Transform flyOverTransform;
    [SerializeField] GameObject cam2Obj;
    ...
    private void Awake()
    {
        ...
        StartCoroutine(FlyOverAnim());
    }

    IEnumerator FlyOverAnim()
    {
        golfTitle.enabled = false;
        golfBG.enabled = false;
        instructionsTitle.color = new Color(instructionsTitle.color.r, instructionsTitle.color.g, instructionsTitle.color.b, 0);
        instructionsBG.color = new Color(instructionsBG.color.r, instructionsBG.color.g, instructionsBG.color.b, 0);

        float timer = 0, totalTime = 145;
        Vector3 startingPos = new Vector3(0, 1450f, 0);
        flyOverTransform.localPosition = startingPos;

        yield return new WaitForSecondsRealtime(24f/60f);
        while(timer <= totalTime)
        {
            flyOverTransform.localPosition = Vector3.Lerp(startingPos, new Vector3(0, -65f, 0), timer / totalTime);
            timer++;
            if (timer == totalTime - 25 || ControlsLogic.flyOverFastTap)
                StartCoroutine(FadeImageIn(whiteSquare, 15));
            if (ControlsLogic.flyOverFastTap)
                break;

            yield return new WaitForFixedUpdate();
        }
        int waitAmount;
        if (timer < totalTime - 25) // started early
            waitAmount = 25;
        else
            waitAmount = (int)totalTime - (int)timer;
        ...
        cam2Obj.SetActive(false);
        StartCoroutine(FadeImageOut(whiteSquare));

        golfTitle.enabled = true;
        golfBG.enabled = true;
        instructionsTitle.color = new Color(instructionsTitle.color.r, instructionsTitle.color.g, instructionsTitle.color.b, 1);
        instructionsBG.color = new Color(instructionsBG.color.r, instructionsBG.color.g, instructionsBG.color.b, 1);
